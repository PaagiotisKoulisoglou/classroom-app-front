...
// Optional env helper that returns empty string when missing to avoid crashing the app at startup
...
const getEnvVarOrDefault = (key: string, fallback: string): string => {
    const value = import.meta.env[key];
    return (typeof value === "string" && value.length > 0) ? value : fallback;
};

export const BACKEND_BASE_URL = getEnvVarOrDefault("VITE_BACKEND_BASE_URL", "http://localhost:8000/api");
